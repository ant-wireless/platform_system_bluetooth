...
unsigned char ucStateBt = STATE_OFF;
unsigned char ucStateAnt = STATE_OFF;

static char NO_SCAN_REQ[] = {0x00,0x00,0x00,0x00,0x00,0x00};

/* Prototypes */
int hci_enable();
int hci_disable();
int hci_is_enabled();
static int bt_stop_scan();
...
int hci_enable()
{
    ...
    for (attempt = 1000; attempt > 0;  attempt--) {
        ...
        if (!ioctl(hci_sock, HCIDEVUP, HCI_DEV_ID)) {
            ...
            // HCI DEV already UP successful
            // Handle case where the other device has already enabled
            break;
        }
        close(hci_sock);
...
int bt_disable() {
    ...
    if (ucStateAnt == STATE_ON) {
        if (bt_stop_scan() < 0) goto out;
    } else if (ucStateAnt == STATE_OFF) {
...
int bt_is_enabled() {
    ...
    if (ucStateBt == STATE_ON)
    {
        ret = hci_is_enabled();
        if (ret == 0) ucStateBt = STATE_OFF;
    }
...
static int bt_stop_scan() {
    ...
    if (hci_sock >= 0)
    {
        if (ioctl(hci_sock, HCISETSCAN, (unsigned long)&NO_SCAN_REQ) < 0) {
            LOGE("Error stopping bluetooth's SCAN. errno = %s", strerror(errno));
        }
        else
        {
            ret = 0;
